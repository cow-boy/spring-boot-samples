...
import com.core.config.Resources;
import com.core.util.IDCardUtil;
...
public abstract class Assert {
	...
	/** 注册或找回 */
	public static void getMsgType(String text, String key) {
		if (text == null) {
			throw new IllegalArgumentException(getMessage(key + "_IS_NULL"));
		}
		if (!"register".equals(text) && !"forget".equals(text) && !"binding".equals(text) && !"fbinding".equals(text) && !"unbinding".equals(text) && !"cash".equals(text)) {
			throw new IllegalArgumentException(getMessage(key));
		}
	}

	/** 账户类型 */
	public static void accountType(Integer text, String key) {
		if (text == null) {
			throw new IllegalArgumentException(getMessage(key + "_IS_NULL"));
		}
		if (text != 2 && text != 3) {
	...
	/** 充值类型 */
	public static void rechargeType(Integer text, String key) {
		if (text == null) {
			throw new IllegalArgumentException(getMessage(key + "_IS_NULL"));
		}
		if (text != 1 && text != 2) {
